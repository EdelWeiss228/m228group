...
bool Tree::remove(AbstractTree::Iterator *iter, int leaf_only){
    ...
    if (empty()||&_Root==inputIter->returnNode())
        return false;
    ...
    if(inputIter->hasNext())
        inputIter->goToNext();
    if (!(tmpNode->isLeaf()))
        _NumberOfElems -= tmpNode->removeChildren(_memory);
    bool flag = false;
    ...
    if(flag2){
        List::ListIterator *rootChildIter = static_cast<List::ListIterator*>(_Root.children.newIterator());
        while (rootChildIter->hasNext()){
            void* elem = rootChildIter->getElement(tmpNode->leafSize);
            Node* child = static_cast<Node*>(elem);
            if(child!=nullptr){
                new (inputIter)Iterator{&_Root, child};
                break;
            }
...
Container:: Iterator* Tree::find(void *elem, size_t size){
    if((elem==nullptr)||(size==0)||(_NumberOfElems == 0))
        return nullptr;
    Node* result = _Root.find(size, elem);
    if (result==nullptr)
        return nullptr;
...
Tree::Iterator* Tree::newIterator() {
    return new Tree::Iterator(&_Root, _Root.getFirst());
}
...
bool Tree::Iterator::goToParent(){
    if(_curNode!=nullptr||_curNode->parent!=nullptr){
        Node* parent = _curNode->parent;
        _curNode = parent;
        return true;
    }
    return false;
...
bool Tree::Iterator::goToChild(int child_index){
    if (_curNode == nullptr)
        return false;
    List::ListIterator *childIter = static_cast <List::ListIterator*>(_curNode->children.newIterator());
    size_t index = 0;
    while (childIter->hasNext()){
        if (index == child_index){
            void* childElem = childIter->getElement(_curNode->leafSize);
...
void* Tree::Iterator::getElement(size_t &size){
    ...
    size = _curNode->leafSize;
    return _curNode->leaf;
}

bool Tree::Iterator::hasNext(){
    if ((_curNode==_Root)||(_curNode==nullptr))
        return false;
    size_t index = _curNode->parent->rightElemIndex(_curNode);
    if((_Root==_curNode->parent)&&((pow(2, 64)-1))==index)
        return false;
    return true;
}
...
void Tree::Iterator::goToNext(){
    ...
    bool flag = false;
    size_t index = 0;
    List::ListIterator* childIter = static_cast <List::ListIterator*>(_curNode->parent->children.newIterator());
    while (childIter->hasNext()){
        void* childElem = childIter->getElement(_curNode->leafSize);
        if (static_cast <Node*>(childElem)==_curNode){
            flag = true;
            break;
        }
        index++;
...
Tree::Iterator *Tree:: createIterator(Tree:: Node *node){
    return new Tree::Iterator(&_Root, node);
}
#endif